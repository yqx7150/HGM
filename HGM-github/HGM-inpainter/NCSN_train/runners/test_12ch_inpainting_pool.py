...
#CUDA_VISIBLE_DEVICES=1 python3.5 separate_ImageNet.py --model ncsn --runner Test_12ch_inpainting_pool --config anneal_bedroom_12ch_pool.yml --doc bedroom_pool_12ch_256 --test --image_folder output/output_inpainter_12ch_pool/6/result

__all__ = ['Test_12ch_inpainting_pool']
...
def write_Data(result_all,i):
    with open(os.path.join('./output/output_inpainter_12ch_pool/6/result/',"psnr"+".txt"),"w+") as f:
...
class Test_12ch_inpainting_pool():
    def __init__(self, args, config):
        self.args = args
        self.config = config

    def test(self):
        states = torch.load(os.path.join(self.args.log, 'checkpoint_370000.pth'), map_location=self.config.device)   #default=24w
        ...
        for j,file_path in enumerate(files_list):
            ...
            mask = cv2.imread('./mask_256/6.png')/255.0       #0.085*0.00006  checkpoint_190000
            ...
            x0 = nn.Parameter(torch.Tensor(samples,12,128,128).uniform_(-1,1)).cuda()
            x01 = x0.clone()
            
            step_lr=0.7*0.00003   # 0.03*0.00006   0.1*0.00003
