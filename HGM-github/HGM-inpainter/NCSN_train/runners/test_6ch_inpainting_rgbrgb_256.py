...
#CUDA_VISIBLE_DEVICES=1 python3.5 separate_ImageNet.py --model ncsn --runner Test_6ch_inpainting_rgbrgb_256 --config anneal_bedroom_6ch_256.yml --doc bedroom_rgbrgb_6ch_ngf256_00005 --test --image_folder aaa         #  bedroom_rgbrgb_6ch 

#CUDA_VISIBLE_DEVICES=1 python3.5 separate_ImageNet.py --model ncsn --runner Test_6ch_inpainting_rgbrgb_256 --config anneal_bedroom_6ch.yml --doc bedroom_rgbrgb_6ch --test --image_folder aaa/6ch

__all__ = ['Test_6ch_inpainting_rgbrgb_256']
...
def write_Data(result_all,i):
    with open(os.path.join('./aaa/6ch/',"psnr"+".txt"),"w+") as f:
...
class Test_6ch_inpainting_rgbrgb_256():
    def __init__(self, args, config):
        self.args = args
        self.config = config

    def test(self):
        states = torch.load(os.path.join(self.args.log, 'checkpoint_240000.pth'), map_location=self.config.device) #checkpoint_240000
        ...
        for j,file_path in enumerate(files_list):
            ...
            image = np.concatenate((image_gt,image_gt),2)
            #cv2.imwrite('./output_inpainter_random_mask/image_gt.png',image_gt)
            mask = cv2.imread('./mask_256/7.png')/255.0       #0.085*0.00006  checkpoint_190000

            '''arr = np.ones((256,256))
            mask = compute_mask(arr,rate=0.70)
            ...
            image_inpainter = np.zeros(image.shape).astype(np.float32)
            image_inpainter =  image * mask
            
            #cv2.imwrite('./output_inpainter_6ch_rgbrgb/image_inpainter_{}.png'.format(j),image_inpainter[:,:,0:3])
            #assert False   
               
            '''img = torch.tensor(image.transpose(2,0,1),dtype=torch.float32).unsqueeze(0) / 255.0
            ...
            #net  random  wordmask2_256  bedroom_rgbrgb_6ch checkpoint_240000  0.1*0.00003
            #Noise amounts
            ...
            for idx, sigma in enumerate(sigmas):
                ...
                for step in range(n_steps_each):
                    ...
                    x_ =  ((x0[:,0:3,...]+x0[:,3:6,...])/2).squeeze(0).cpu().detach().numpy().transpose(1,2,0)
                    plt.ion()
                    plt.imshow(x_)
                    plt.show()
                    plt.pause(0.3)
