...
#CUDA_VISIBLE_DEVICES=1 python3.5 separate_ImageNet.py --model ncsn --runner Test_6ch_demoscaicking_copy --config anneal.yml --doc MSR_64_6ch --test --image_folder try_output_copy/Kodak_output

#import imutils
__all__ = ['Test_6ch_demoscaicking_copy']
def show(image):
    plt.figure(1)
...
def write_Data(result_all,i):
    with open(os.path.join('./try_output_copy/Kodak_output/',"psnr_Kodak"+".txt"),"w+") as f:
...
class Test_6ch_demoscaicking_copy():
    ...
    def test(self):
        ...
        scorenet.eval()
        # get data 
        files_list = glob.glob('./reference_image/Kodak/kodim01.png')
        #files_list = glob.glob('./McM/*.tif')
        files_list.sort()
        length = len(files_list)
        
        result_all = np.zeros([13,3])
        ...
        for j,file_path in enumerate(files_list):
            ...
            step_lr=0.25*0.00003    #MSR 0.25**0.00003             #0.035*0.00003 
            ...
            for idx, sigma in enumerate(sigmas):
                ...
                for step in range(n_steps_each):
                    ...
                    if max_psnr < psnr :
                        ...
                        imsave('./try_output_copy/Kodak_output/'+'img_{}_Rec_'.format(j)+'.png',(255.0*x_rec).astype(np.uint8))
                    if max_ssim < ssim:
                        result_all[j,1] = ssim
                        max_ssim = ssim
